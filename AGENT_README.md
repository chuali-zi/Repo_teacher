...
Repo Tutor 是一个 **本地单用户、只读、教学型** Web 应用。用户输入仓库路径/GitHub URL → 确定性静态分析 → LLM 生成教学报告 → 多轮对话持续深入。
...
| M5 | `backend/m5_session/` | 会话编排、状态机、教学状态、SSE 映射 | 否 |
| M6 | `backend/m6_response/` | Prompt 构建、LLM 流式调用、结构化解析 | **是** |
...
POST /api/chat (202)
  ...
  → M5 构建 PromptBuildInput (骨架切片 + 教学上下文)
  → M6 LLM 流式调用 + 结构化解析
...
3. **M6 不读写 SessionContext**：只消费 `PromptBuildInput`。
4. **敏感文件只记录存在**：正文不进入分析/SSE/日志/Prompt。
5. **安全只读**：不执行代码、不安装依赖、不修改文件。
6. **前端服务端驱动**：视图状态来自 DTO/SSE，不本地推断。
7. **命名源**：`backend/contracts` + `frontend/src/types/contracts.ts`。
8. **不扩展规范**：不添加规范外的路由/消息类型/枚举值/状态转换。
