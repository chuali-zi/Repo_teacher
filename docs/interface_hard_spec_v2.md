...
> **对应上位文档**：`PRD_v5_agent.md`, `interaction_design_v1.md`, `technical_architecture_v2.md`, `data_structure_design_v2.md`  
> **修订来源**：审计 `interface_hard_spec_v1.md`  
...
| 编号 | 章节 | 说明 |
|------|------|------|
| [API2-00](#api2-00) | 审计结论 | v1 是否可接受、v2 修正范围 |
...
v2 不推翻 v1 的方向，而是把 v1 的原则补成可执行契约：
...
3. 模块职责、运行流程、技术选型：`technical_architecture_v2.md`
4. 内部数据对象、枚举、生命周期：`data_structure_design_v2.md`
5. 本文：只裁决接口层未明确处，不得扩展 v1 范围外能力。

### v2 裁决

1. 前后端通信只使用 `HTTP REST + SSE`。`technical_architecture_v2.md` 中“HTTP/WebSocket”的表述以技术栈表和 ADR-02 的 SSE 裁决为准。
...
6. 本文新增接口层参数错误码 `invalid_request`。这是 v1 接口层必需错误码；若后续修订 `data_structure_design_v2.md`，应同步补入 `ErrorCode`。
...
本节定义前后端传输对象。内部实现可以使用 `data_structure_design_v2.md` 的对象，但对外只能暴露本文 DTO 中定义的字段。
...
3. 如果旧会话存在，服务端必须先执行 [DS2-13](data_structure_design_v2.md#ds2-13) 清理顺序。
...
3. 流式失败时必须发送 `error`；如已有部分文本，最终 `MessageDto.error_state.partial_text_available=true`。
4. 旧会话被清理时必须关闭旧聊天流。
...
4. `scenario=depth_adjustment`：只改变 `depth_level`，不清空学习目标。
...
*本文作为 `interface_hard_spec_v1.md` 的可执行修订版。后续脚手架 Agent 应优先引用 `docs/interface_hard_spec_v2.md`。若实现中发现本文与上位文档冲突，应回改本文或上位数据结构文档，而不是在代码中发明第三套契约。*
