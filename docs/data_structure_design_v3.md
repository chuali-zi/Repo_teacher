# Repo Tutor — 核心数据结构设计 v3

> **文档类型**：数据结构规格
> **对应架构**：`technical_architecture_v3.md`
> **对应 PRD**：`PRD_v5_agent.md`
> **对应交互设计**：`interaction_design_v1.md`
> **对应接口**：`interface_hard_spec_v3.md`
> **修订来源**：审计 `data_structure_design_v1.md`，并同步接口 v3 的错误码裁决
> **适用范围**：第一版单 Agent、只读、教学型 Repo Tutor
> **日期**：2026-04-12

---

> v3 为 v2 的兼容修订版，保留 `DS2-*` 章节锚点，避免已存在的跨文档链接失效。v3 修复会导致实现分裂的硬契约问题，并同步 M5 教学状态追踪（教学计划/学生状态/教师日志/教学决策）已落地的数据结构。

## 索引

| 编号 | 章节 | 说明 |
|------|------|------|
| [DS2-00](#ds2-00) | 审计结论 | v1 偏移与 v2/v3 修正范围 |
...
`data_structure_design_v1.md` 的产品方向和主实体拆分基本贴合 `technical_architecture_v3.md`、`PRD_v5_agent.md` 与 `interaction_design_v1.md`，但作为后续实现 Agent 的输入不够可靠，存在不可接受的执行风险。
...
v2 保留 v1 的 8 个核心实体，但补齐所有引用类型，并新增 M6 prompt 与回答数据契约和验证规则。v3 同步接口层必需错误码 `invalid_request`。本文不定义接口规范、路由实现细节、协议细节、前端组件 props 或数据库表。

### 架构口径裁决

`technical_architecture_v2.md` 曾保留过少量旧口径表述。该问题已由 `technical_architecture_v3.md` 与 `interface_hard_spec_v3.md` 裁决；本文只记录边界，不定义接口。
...
- M6 只能消费 `PromptBuildInput`。其中 `TeachingSkeleton` 和 `topic_slice` 继续作为内部教学状态输入；真正进入 prompt 的静态分析参考应优先通过 `LlmToolContext` 的工具目录和工具结果表达，不反向修改 `AnalysisBundle`。
- M1-M4 产物属于工具参考，不是 LLM 回答边界；工具证据不足时允许推断，但必须标注候选或不确定性。
...
| LLM 工具层 | M1-M4 产物、教学状态、本轮主题切片 | `LlmToolContext` | DS2-09 |
...
| `progress_steps` | `list[ProgressStepStateItem]` | 当前分析阶段快照，供 `GET /api/session` 和分析流重连恢复 |
...
- `progress_steps` 是稳定快照，不得只靠 `runtime_events` 临时重建；`GET /api/session` 在 `accessing`, `analyzing`, `analysis_error`, `chatting` 下都必须能返回最新快照。
- `status=idle` 时，`progress_steps` 必须为空列表。
...
| `sub_status` | `ConversationSubStatus \| null` | 对话态子状态；仅 `SessionContext.status=chatting` 时允许非空 |
| `teaching_plan_state` | `TeachingPlanState \| null` | 教学计划表：按 LearningGoal 排列的步骤序列 |
| `student_learning_state` | `StudentLearningState \| null` | 学生学习状态：按主题记录覆盖度和困惑信号 |
| `teacher_working_log` | `TeacherWorkingLog \| null` | 教师工作日志：当前教学目标、风险备注、最近决策 |
| `current_teaching_decision` | `TeachingDecisionSnapshot \| null` | 本轮教学决策快照 |
| `teaching_debug_events` | `list[TeachingDebugEvent]` | 教学调试事件序列（上限 80 条），仅内部使用 |

### `ConversationState` 默认约束

- 会话创建或清理后，`current_learning_goal` 默认置为 `overview`。
- 会话创建或清理后，`current_stage` 默认置为 `not_started`。
- `SessionContext.status!=chatting` 时，`conversation.sub_status` 必须为 `null`。
- M5 启动多轮回答时，`conversation.sub_status` 依次进入 `agent_thinking`、`agent_streaming`，回答完成后回到 `waiting_user`。
...
M5 在调用 M6 前组装 `LlmToolContext`，把 M1-M4 产物和补充只读读取能力表达成 LLM 可消费的工具目录和工具结果。该结构仅供后端 prompt 构建使用，不属于外部 DTO。

#### `LlmToolDefinition`

| 字段 | 类型 | 说明 |
|------|------|------|
| `tool_name` | `str` | 工具名，如 `m3.get_entry_candidates` |
| `source_module` | `str` | 来源模块或子模块 |
| `description` | `str` | 给 LLM 看的工具用途说明 |
| `input_schema` | `dict` | 参数约定；预组装结果工具可为空 |
| `output_contract` | `str \| null` | 输出形态说明 |
| `safety_notes` | `list[str]` | 安全和候选措辞约束 |
| `deterministic` | `bool` | 是否确定性工具；教学状态快照可为 `false` |

#### `LlmToolResult`

| 字段 | 类型 | 说明 |
|------|------|------|
| `result_id` | `str` | 工具结果 ID |
| `tool_name` | `str` | 对应工具名 |
| `source_module` | `str` | 来源模块 |
| `summary` | `str` | 结果摘要 |
| `payload` | `dict` | 工具输出载荷，必须已清洗敏感字段 |
| `reference_only` | `bool` | 固定为 `true`，提醒 LLM 这是参考 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `policy` | `str` | 工具使用策略：静态分析只是参考，推断必须标注不确定性 |
| `tools` | `list[LlmToolDefinition]` | 可用工具目录 |
| `tool_results` | `list[LlmToolResult]` | 本轮预组装工具结果 |
...
| `teaching_skeleton` | `TeachingSkeleton` | 教学骨架，供 M5/M6 内部保持首轮报告结构和教学状态兼容 |
| `topic_slice` | `list[TopicRef]` | 根据意图抽取的主题切片，供教学决策和工具上下文构建使用 |
| `tool_context` | `LlmToolContext \| null` | LLM 可消费的工具目录和工具结果；新路径必须填充 |
...
以下结构由 M5 `teaching_state` 模块管理，在首轮报告完成后初始化，每轮对话后更新。这些结构仅在后端内存和 M6 prompt 中使用，不暴露在外部 DTO 中。
...
| `step_id` | `str` | 步骤 ID |
| `title` | `str` | 步骤标题 |
| `goal` | `LearningGoal` | 对应学习目标 |
| `target_scope` | `str` | 目标范围描述 |
| `reason` | `str` | 为什么安排此步骤 |
| `expected_learning_gain` | `str` | 预期学习收益 |
| `status` | `TeachingPlanStepStatus` | planned / active / completed / deferred / cancelled |
| `priority` | `int` | 优先级 |
| `depends_on` | `list[str]` | 依赖的步骤 ID |
| `source_topic_refs` | `list[TopicRef]` | 来源主题引用 |
| `adaptation_note` | `str \| null` | 适配说明 |
...
|------|------|------|
| `plan_id` | `str` | 教学计划 ID |
| `generated_from_skeleton_id` | `str` | 生成来源的骨架 ID |
| `current_step_id` | `str \| null` | 当前执行中的步骤 |
| `steps` | `list[TeachingPlanStep]` | 步骤序列 |
| `update_notes` | `list[str]` | 更新记录 |
| `updated_at` | `datetime` | 最近更新时间 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `topic` | `LearningGoal` | 学习主题 |
| `coverage_level` | `StudentCoverageLevel` | 覆盖度：unseen / introduced / partially_grasped / needs_reinforcement / temporarily_stable |
| `confidence_of_estimate` | `ConfidenceLevel` | 对估算的置信度 |
| `last_explained_at_message_id` | `str \| null` | 最近讲解所在消息 |
| `student_signal` | `str \| null` | 学生信号（如困惑表达） |
| `likely_gap` | `str \| null` | 可能的知识缺口 |
| `recommended_intervention` | `str \| null` | 建议干预方式 |
...
|------|------|------|
| `state_id` | `str` | 状态 ID |
| `topics` | `list[StudentLearningTopicState]` | 各主题的学习状态 |
| `update_notes` | `list[str]` | 更新记录 |
| `updated_at` | `datetime` | 最近更新时间 |
...
|------|------|------|
| `log_id` | `str` | 日志 ID |
| `current_teaching_objective` | `str` | 当前教学目标 |
| `why_now` | `str` | 为什么当前是这个目标 |
| `active_topic_refs` | `list[TopicRef]` | 当前活跃的主题引用 |
| `current_plan_step_id` | `str \| null` | 当前教学计划步骤 |
| `planned_transition` | `str \| null` | 计划中的下一步转换 |
| `student_risk_notes` | `list[str]` | 学生风险备注 |
| `recent_decisions` | `list[str]` | 最近决策记录 |
...
| `scenario` | `PromptScenario` | 场景 |
| `user_message_summary` | `str \| null` | 用户消息摘要 |
| `selected_action` | `TeachingDecisionAction` | 选定的教学动作 |
| `selected_plan_step_id` | `str \| null` | 选定的计划步骤 |
| `selected_plan_step_title` | `str \| null` | 选定步骤标题 |
| `teaching_objective` | `str` | 本轮教学目标 |
| `decision_reason` | `str` | 决策理由 |
| `student_state_notes` | `list[str]` | 学生状态备注 |
| `planned_transition` | `str \| null` | 计划转换 |
| `topic_refs` | `list[TopicRef]` | 关联主题引用 |
| `created_at` | `datetime` | 创建时间 |
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `debug_event_id` | `str` | 调试事件 ID |
| `event_type` | `TeachingDebugEventType` | 事件类型 |
| `occurred_at` | `datetime` | 发生时间 |
| `message_id` | `str \| null` | 关联消息 |
| `plan_step_id` | `str \| null` | 关联计划步骤 |
| `summary` | `str` | 摘要 |
| `details` | `dict` | 详细信息 |

#### 教学状态约束

- `teaching_plan_state` 在首轮报告完成后初始化，切换仓库时清空。
- `student_learning_state` 在首轮报告完成后初始化各主题为 `unseen`，随对话更新覆盖度。
- `teacher_working_log` 每轮对话后滚动更新，保留最近 3 轮摘要。
- `current_teaching_decision` 在每轮对话开始前生成，用于指导 M6 prompt 构建。
- `teaching_debug_events` 上限 80 条，超出时丢弃最早事件。
- 以上所有教学状态仅注入 M6 prompt，不在 `GET /api/session` 响应中暴露。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `step_key` | `ProgressStepKey` | 分析步骤 |
| `step_state` | `ProgressStepState` | 当前步骤状态 |

约束：

- 这是 `SessionContext.progress_steps` 的稳定快照对象，不是瞬时事件。
- 创建新会话时必须按固定顺序初始化全部步骤：`repo_access`, `file_tree_scan`, `entry_and_module_analysis`, `dependency_analysis`, `skeleton_assembly`, `initial_report_generation`，初值均为 `pending`。
- 任一步骤状态变化后，M5 必须同时更新对应 `RuntimeEvent` 和 `SessionContext.progress_steps`，避免 `GET /api/session` 与 SSE 重连看到不一致状态。
...
2. 用户提交仓库后创建或重置 `SessionContext`，状态置为 `accessing`，并初始化 `progress_steps`。
...
7. M5 初始化教学状态，并在调用 M6 前组装 `LlmToolContext`。
8. M6 首轮报告流式完成后，写入 `MessageRecord(message_type=initial_report)`，并将 `progress_steps` 全部置为最终状态。
9. M5 将 `status` 置为 `chatting`，`conversation.sub_status` 置为 `waiting_user`。
10. 多轮追问中追加 `MessageRecord`，每轮重新组装本轮 `LlmToolContext`，并更新 `ConversationState`。
...
5. 清空 `repository`, `file_tree`, `analysis`, `teaching_skeleton`, `active_degradations`, `last_error`, `progress_steps`。
6. 重置 `conversation`，其中 `current_learning_goal=overview`, `current_stage=not_started`, `depth_level=default`, `sub_status=null`。
7. 将 `status` 置为 `idle`。

### 缓存策略

- 同一会话、同一仓库内，多轮对话复用 M2/M3/M4 产物，并按本轮目标重新投影 `LlmToolContext`。
- 不做磁盘缓存。
- 不做跨会话缓存。
- 不缓存敏感文件正文。
...
5. M6 每轮回答必须符合 OUT-11 六段式结构，且保留下一步建议；静态分析只作为 `LlmToolContext` 的工具参考。
...
13. `GET /api/session` 所需的 `progress_steps` 必须来自 `SessionContext.progress_steps` 稳定快照，不能只靠回放 `runtime_events` 推导。
14. 教学状态（`teaching_plan_state`、`student_learning_state`、`teacher_working_log`、`current_teaching_decision`、`teaching_debug_events`）仅在后端内存和 M6 prompt 中使用，不暴露在外部 DTO 中。
15. `teaching_debug_events` 上限 80 条，超出时丢弃最早事件。
16. 切换仓库时教学状态与其他会话状态一并清空。
17. `LlmToolContext` 不得包含 `root_path`、`real_path`、内部错误详情或敏感文件正文；`repo.read_file_excerpt` 和 `repo.search_text` 只允许读取 `FileNodeStatus.NORMAL` 的非敏感文件。
